...
from .connectors.mt5_simulator_connector import PlatformConnector, AccountInfoConnector, TerminalInfoConnector, SymbolConnector
...
# This will be the class you would call like: import Mt5SimulatorWrapper as mt5

class Mt5SimulatorWrapper():

    SharedData()  # Important: 1st thing to execute. This is to execute the init and load the default global data
    ...
    def initialize(path:str='', login:int=0, password:str='', server:int=0, timeout:int=60000, portable:bool=False) -> bool:
        return PlatformConnector.initialize(path=path, login=login, password=password, server=server, timeout=timeout, portable=portable)
    
    @staticmethod
    def login(login:int=0, password:str='', server:int=0, timeout:int=60000) -> bool:
        return PlatformConnector.login(login=login, password=password, server=server, timeout=timeout)
    
    @staticmethod
    def shutdown() -> None:
        PlatformConnector.shutdown()
    
    @staticmethod
    def version() -> tuple:
        return PlatformConnector.version()
    
    @staticmethod
    def last_error() -> tuple:
        return PlatformConnector.last_error()
    
    @staticmethod
    def account_info() -> AccountInfo:
        return AccountInfoConnector.account_info()
    
    @staticmethod
    def terminal_info() -> TerminalInfo:
        return TerminalInfoConnector.terminal_info()
    
    @staticmethod
    def symbols_total() -> int:
        return SymbolConnector.symbols_total()

    @staticmethod
    def symbols_get(group: str = "*") -> Tuple[SymbolInfo, ...]:
        return SymbolConnector.symbols_get(group=group)
    
    @staticmethod
    def symbol_info(symbol: str) -> SymbolInfo:
        return SymbolConnector.symbol_info(symbol=symbol)
    
    @staticmethod
    def symbol_info_tick(symbol: str) -> Tick:
        """ NOT FULLY IMPLEMENTED"""
        return SymbolConnector.symbol_info_tick(symbol=symbol)
    
    @staticmethod
    def symbol_select(symbol: str, enable: bool = True) -> bool:
        """ NOT FULLY IMPLEMENTED"""
        return SymbolConnector.symbol_select(symbol=symbol, enable=enable)
